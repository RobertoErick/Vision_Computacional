...
if imagen is None:
    ...
    np.savetxt('Original_Shifting_diag_der_aba_a_izq_arr.csv', gray_image, delimiter=',', fmt='%d')
    print("La matriz de la imagen en escala de grises se ha guardado en 'Original_Shifting_diag_der_aba_a_izq_arr.csv'.")
    ...
    np.savetxt('Shifting_diag_der_aba_a_izq_arr.csv', gray_copy, delimiter=',', fmt='%d')

    # Mostrar la imagen en blanco y negro
    cv2.imshow("Imagen a Blanco y negro", gray_image)
    cv2.imshow("shifting diagonal derecha abajo a izquierda arriba", gray_copy)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite("shifting diagonal derecha abajo a izquierda arriba.png", gray_copy)
