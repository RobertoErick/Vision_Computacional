...
image = cv2.imread('imagen fondo rosa.png')  # Reemplaza con la ruta de tu imagen
if image is None:
...
# PASO NUEVO: Función de acomodo del perfil AVP
def acomodar_avp(perfil):
    # Normalizar el perfil para evitar variaciones de tamaño
    perfil_normalizado = (perfil - np.min(perfil)) / (np.max(perfil) - np.min(perfil))
    # Ajuste de longitud si es necesario, puede interpolarse o recortarse
    longitud_objetivo = 100  # Define la longitud objetivo para el perfil
    perfil_acomodado = np.interp(np.linspace(0, len(perfil_normalizado)-1, longitud_objetivo),
                                 np.arange(len(perfil_normalizado)), perfil_normalizado)
    return perfil_acomodado

# Aplicar la función de acomodo
perfil_acomodado = acomodar_avp(perfil_continuo)

# PASO NUEVO: Aplicar la Transformada Discreta del Coseno (DCT) al perfil AVP acomodado
dct_profile = dct(perfil_acomodado, type=2)
...
# Mostrar el perfil AVP continuo y acomodado
plt.figure(figsize=(10, 4))
plt.plot(perfil_acomodado, color='blue', lw=1)
plt.title("Perfil AVP Continuo de la Hoja (Acomodado)")
